...
- `sdshdr5`
![sdshdr5](/img/sdshdr5.png)

- `sdshdr8`
![sdshdr8](/img/sdshdr8.png)
...
上述函数均委托位于`sdsalloc.h`中的宏函数实现内存管理，又委托`zmalloc.h`中的具体定义实现。底层可以为`tcmalloc`、`jemalloc`或标准库实现，通过`USE_TCMALLOC`、`USE_JEMALLOC`、`__APPLE__`和`__GLIBC__`等宏开关控制，具体分析参见章节[Redis 内存管理](./memory_management.md)。
...
![sds_sdshdr_transform](/img/sds_sdshdr_transform.png)
...
## sds 对象的释放
sds 对象主要通过`sds.h:268#sds_free(void *)`释放内存空间，最终委托 Redis 内存管理中的 `zfree`函数实现，也可以直接调用`zfree`释放内存。详细内容参见章节[Redis 内存管理](./memory_management.md)。
