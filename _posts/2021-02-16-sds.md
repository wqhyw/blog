...
- `sdshdr5`
![sdshdr5]({{ site.url }}{{ site.baseurl }}/img/sdshdr5.png)

- `sdshdr8`
![sdshdr8]({{ site.url }}{{ site.baseurl }}/img/sdshdr8.png)
...
上述函数均委托位于`sdsalloc.h`中的宏函数实现内存管理，又委托`zmalloc.h`中的具体定义实现。底层可以为`tcmalloc`、`jemalloc`或标准库实现，通过`USE_TCMALLOC`、`USE_JEMALLOC`、`__APPLE__`和`__GLIBC__`等宏开关控制，具体分析参见章节[Redis 内存管理]({{ site.url }}{{ site.baseurl }}/memory_management)。
...
![sds_sdshdr_transform]({{ site.url }}{{ site.baseurl }}/img/sds_sdshdr_transform.png)
...
```mermaid
graph TD
A([START])-->B[由 initlen 获取 sds_type]
subgraph "obtain sds type"
  B-->C{sys_type 为 SDS_TYPE_5}
  C-->|Y|D[使用 sds_type 指定为 SDS_TYPE_8]
  C-->|N|E
  D-->E[由 sds_type 获取对象头大小至 hdrlen]
end
subgraph "allocate memory"
  E-->F[分配大小为 hdrlen+initlen+1 的内存空间至 sh]
  F-->G{init 内容为 SDS_NOINIT}
  G-->|Y|H[init 指为 NULL]
  I-->|Y|J[以0填充 sh 内容]
  G-->|N|I{init 为空}
  I-->|N|K{sh 为 NULL}
...
subgraph "set header"
  H-->L
  J-->L
  K-->|N|L[获取 buf 域地址和 flag 域地址]
  L-->M[设置 len 域和 alloc 域为 initlen]
  M-->N[设置 flag 域为sds_type]
end
subgraph "set content"
  N-->O{initlen>0 且 init 不为 NULL}
  O-->|Y|P[init 内容 copy 至 buf 域中]
  O-->|N|Q[设置 buf 域最后一位为\0]
  P-->Q
end
K-->|Y|ZX[返回NULL]
Q-->ZY[返回 buf 域地址]
...
## sds 对象的释放
sds 对象主要通过`sds.h:268#sds_free(void *)`释放内存空间，最终委托 Redis 内存管理中的 `zfree`函数实现，也可以直接调用`zfree`释放内存。详细内容参见章节[Redis 内存管理]({{ site.url }}{{ site.baseurl }}/memory_management)。
...
```mermaid
graph TD
A([START])-->B[获取缓冲区当前剩余空间 avail]
subgraph "calculte new length"
  B-->C[获取当前 sds_type]
  C-->D{avail>=add_len}
  D-->|N|E[获取当前缓冲区内容长度 len]
  E-->F[len+addlen 为 newlen]  
  F-->G{newlen 小于 SDS_MAX_PREALLOC}
  G-->|Y|H[newlen*=2]
  G-->|N|I[newlen+=SDS_MAX_PREALLOC]
  I-->J[根据 newlen 计算新 sds_type]
  H-->J
  J-->KK[根据 sds_type 计算对象头长度 hdrlen]
end
subgraph "set new sds"
  KK-->K{sds_type 不变}
  K-->|Y|L[调用 realloc 过程扩容缓冲区大小至 hdrlen+newlen+1]
  L-->M{扩容结果为 NULL}
  M-->|N|N[设置 alloc 域为 newlen]
  K-->|N|O[新分配空间 hdrlen+newlen+1]
  O-->P{分配空间为空}
  P-->|N|Q[拷贝原 sds 对象至新地址]
  Q-->R[释放原 sds 对象]
  R-->S[设置 sds_type 至新对象 flag 域]
  S-->T[设置 len 至 新对象 len 域]
  T-->N
end
D-->|Y|ZW[返回原 sds 对象地址]
N-->ZW
