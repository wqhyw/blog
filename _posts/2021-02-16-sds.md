...
* 改变内容时优先扩容，留有空间供后续操作，减少频繁分配内存等操作；
...
|:-:|:-:|:-:|:-:|
|`sds sdsdup(const sds s)`|sds.c:160|复制字符串（深拷贝）|`O()`|
|`sds sdsgrowzero(sds s, size_t len)`|sds.c:379|扩容字符串缓冲区至指定长度|`O()`|
|`sds sdscatlen(sds s, const void *t, size_t len)`|sds.c:397|追加指定长度的二进制数据|`O()`|
|`sds sdscat(sds s, const char *t)`|sds.c:412|追加 C 风格字符串|`O()`|
|`sds sdscatsds(sds s, const sds t)`|sds.c:420|追加`sds`对象|`O()`|
|`sds sdscpylen(sds s, const char *t, size_t len)`|sds.c:426|替换为指定长度的字符串|`O()`|
|`sds sdscpy(sds s, const char *t)`|sds.c:439|替换为 C 风格字符串|`O()`|
|`sds sdscatvprintf(sds s, const char *fmt, va_list ap)`|sds.c:522|按照格式写字符串入缓冲区（依靠标准库宏`vsnprintf`实现）|`O()`|
|`sds sdscatprintf(sds s, const char *fmt, ...)`|sds.c:522|功能同`sdscatvprintf`，GNU 扩展，增加`__attribute__((format(printf, 2, 3)))`校验|`O()`|
|`sds sdscatfmt(sds s, char const *fmt, ...)`|sds.c:600|功能同`sdscatvprintf`，未以来标准库，直接实现部分格式占位符的支持|`O()`|
|`sds sdstrim(sds s, const char *cset)`|sds.c:704|从字符串左右两端去除指定的字符|`O()`|
|`void sdsrange(sds s, ssize_t start, ssize_t end)`|sds.c:735|指定范围截取字符串|`O()`|
|`void sdsupdatelen(sds s)`|sds.c:184|通过标准库函数`strlen`计算字符串长度并重新设置|`O()`|
|`void sdsclear(sds s)`|sds.c:193|字符串首位置`\0`并设置长度为0|`O()`|
|`int sdscmp(const sds s1, const sds s2)`|sds.c:788|比较两个字符串，相等条件为长度相同且内容一致<br />（通过`<string.h>#memcmp(const void *, const void *, size_t)->int`比较内容）|`O()`|
|`sds *sdssplitlen(const char *s, ssize_t len, const char *sep, int seplen, int *count);`|sds.c:816|以子串`sep`为分割符分隔字符串(C 风格字符串和`sds`均可)，返回`sds`数组和长度，以`len`为原字符串长度保证二进制安全|`O()`|
|`void sdsfreesplitres(sds *tokens, int count)`|sds.c:867|释放字符串数组空间|`O()`|
|`void sdstolower(sds s)`|sds.c:764|字符串内英文字母变小写|`O()`|
|`void sdstoupper(sds s)`|sds.c:771|字符串内英文字母变大写|`O()`|
|`sds sdsfromlonglong(long long value)`|sds.c:514|整数转字符串|`O()`|
|`sds sdscatrepr(sds s, const char *p, size_t len)`|sds.c:880|将字符串中不可打印字符转换并显示<br />如`\t`转为`\\t`，`U+000A`转换为`\x0A`|`O()`|
|`sds *sdssplitargs(const char *line, int *argc)`|sds.c:955|通过空白分割符将字符串转换为 token 数组|`O()`|
|`sds sdsmapchars(sds s, const char *from, const char *to, size_t setlen)`|sds.c:1074|转换字符串中的字符<br />如`from`为 `AB`，`to`为`CD`时表示 字符串中 A 替换为 C，B 替换为 D|`O()`|
|`sds sdsjoin(char **argv, int argc, char *sep)`|sds.c:1090|将 C 风格字符串数组通过指定分割符合成一个字符串|`O()`|
|`sds sdsjoinsds(sds *argv, int argc, const char *sep, size_t seplen)`|sds.c:1102|将`sds`数组通过指定分割符合成一个字符串|`O()`|
